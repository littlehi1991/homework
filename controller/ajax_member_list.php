...
switch ($request['method']){
    case 'insert':
        ...
        if (empty($request['account']) or empty($request['name']) or empty($request['birthday']) or empty($request['email']) or (empty($request['sex']) and !isset($request['sex']))) {
            echo json_encode(['status' => '新增失敗，缺少必要欄位', 'des' => '缺少必要欄位']);exit;
        }
        $rs = $method->doInsertProcess($request);
        echo ($rs === true) ? json_encode(['status' => '新增成功', 'des' => '新增成功']) : json_encode(['status' => false, 'des' => '失敗']);exit;
        break;
    case 'update' :
        if (empty($request['account']) or empty($request['name']) or empty($request['birthday']) or empty($request['email']) or (empty($request['sex']) and !isset($request['sex']))) {
            echo json_encode(['status' => '新增失敗，缺少必要欄位', 'des' => '缺少必要欄位']);exit;
        ...
        $rs = ($rs === true) ? json_encode(['status' => '新增成功', 'des' => 'success']) : json_encode(['status' => false, 'des' => 'fail']);
        echo $rs;exit;
        break;
    default:
